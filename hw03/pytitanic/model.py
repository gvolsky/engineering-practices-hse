...
def train(odir, trname, tsname, split_col, rs, train_size):
    ...
    train.fillna(train.iloc[tridx].mean()[["Age", "Fare"]], inplace=True)
    ...
    model.fit(
        train.iloc[tridx][fts_cols],
        train.iloc[tridx]["Survived"],
        eval_set=(train.iloc[cvidx][fts_cols], train.iloc[cvidx]["Survived"]),
        verbose=50
        )
...
if __name__ == "__main__":
    ...
    train(args.odir, args.trname, args.tsname, args.split_col, args.rs, args.train_size)